...
// 全局变量，用于存储和访问YamiBotBoard实例
YamiBotBoard* g_board_instance = nullptr;
...
struct TaskParams {
    YamiBotBoard* board;  // 使用YamiBotBoard*代替void*
...
public:
    ...
    EmojiDisplay(YamiBotBoard* board, esp_lcd_panel_io_handle_t io, esp_lcd_panel_handle_t panel, 
...
class YamiBotBoard : public WifiBoard {
private:
    ...
    void InitializeSsd1306Display() {
        ...
        // 使用自定义的EmojiDisplay类
        display_ = new EmojiDisplay(this, panel_io_, panel_, DISPLAY_WIDTH, DISPLAY_HEIGHT, DISPLAY_MIRROR_X, DISPLAY_MIRROR_Y);
    ...
    // 声明EmojiDisplay为友元类，使其能够访问YamiBotBoard的私有成员
...
void EmojiDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    if (role && strcmp(role, "assistant") == 0 && content && content[0] != '\0') {
        ...
        // 调用YamiBotBoard的ProcessAIResponse方法处理AI回复
...
static void StateMonitorTask(void* arg) {
    ...
    YamiBotBoard* board = params->board;
    if (!board) {
...
static void ProcessAIResponseTask(void* arg) {
    ...
    if (message) {
        ...
        // 直接从YamiBotBoard获取
        if (g_board_instance) {
            // 由于ProcessAIResponseTask是YamiBotBoard的友元函数，可以直接访问私有成员
            emotion_controller = g_board_instance->emotion_controller_;
            ESP_LOGI("AIResponseTask", "从YamiBotBoard获取情感控制器");
        }
